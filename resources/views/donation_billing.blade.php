...
<body>
    ...
    @if (request()->isMethod('post'))
        @php
            $pay_categories = request('pay_categories');
    ...
    <section class="billing">
        ...
        <div class="container">
            ...
            <input type="text" class="form-control" id="your-subject" name="your-subject" placeholder="Leave blank if you are a new member">
                                ...
                                <div class="col-md-3">
                                    @if (is_array($pay_categories) && is_array($amount))
                                        @for ($i = 0; $i < count($pay_categories); $i++)
                                            <input type="hidden" name="pay_categories[]" id="pay_categories{{ $i+1 }}"
                                                value="{{ $pay_categories[$i] }}">
                                            <input type="hidden" name="amount[]" id="amount{{ $i+1 }}"
                                                value="{{ $amount[$i] }}">
                                        @endfor
                                    @endif
